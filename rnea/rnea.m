function [u, f, n] = rnea(q, qd, q_aux_d, qdd, use_gravity, robot_params)
    ...
    num_q = length(robot_params.active_joints);
    ...
    for i = 1:num_joints
        switch joint_types{i}
            case 'revolute'
                ...
                if dim == 1
                    R(:,:,i) = rx(joint_pos(robot_params.q_index(i)));
                elseif dim == 2
                    R(:,:,i) = ry(joint_pos(robot_params.q_index(i)));
                else
                    R(:,:,i) = rz(joint_pos(robot_params.q_index(i)));
                end
            case 'fixed'
                R(:, :, i) = T0(1:3, 1:3, i);
            otherwise
